...
def cli_help(cmd_name, cmd, sig):
    ...
    for k, v in sig.parameters.items():
        ann = v.annotation.__name__ if v.annotation is not inspect.Parameter.empty else ""
        d = v.default if v.default is not inspect.Parameter.empty else None
        log(f"** {k} {ann}: {d}")
...
def cast_n_bind(sig, *args, **kwargs):
    ...
    for k, v in ba.arguments.items():
        ...
        ann = k_sig.annotation
        default = k_sig.default
        if ann is bool or type(default) is bool:
            ba.arguments[k] = v.lower() in ('true', '1', 'yes') if isinstance(v, str) else bool(v)
        elif ann is int or type(default) is int:
            ba.arguments[k] = int(v)
        elif ann is float or type(default) is float:
            ba.arguments[k] = float(v)
        elif ann is Sequence[int]:
            a = v.split(',')
